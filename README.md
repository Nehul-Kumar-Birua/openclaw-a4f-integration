...
### 1. Install OpenClaw
```bash
global install openclaw # or follow the official guide
```
...
```
### 3. Set Your Keys
Edit the file and replace `YOUR_A4F_API_KEY` with your actual key from [A4F.co](https://a4f.co/).
### 4. Run It
...
```bash
eclaw tui --timeout-ms 120000 # Use a higher timeout for large models!
```
## 📸 Screenshots
_(Add a screenshot of your TUI working here!)_

## Step 4: Add a Screenshot
Take a screenshot of your terminal showing the successful chat (especially where it admits it's using Llama 3.3). Upload this to your repo's `docs/` folder and link it in the README.

## Step 5: Push to GitHub
```bash
git init
git add .
git commit -m "Initial commit: OpenClaw A4F integration guide"
git branch -M main
git remote add origin https://github.com/YOUR_USERNAME/openclaw-a4f-integration.git
